...
int main(int argc, char *argv[])
{
   ...
   printf("WITH_WORKERS: Initializing MPI...\n");
   MPI_Init(&argc, &argv);
   
   std::vector<int> excl;
   excl.push_back(0); // Exclude process 0 from the working group.
   
   printf("WITH_WORKERS: Initializing MPMD...\n");
   ...
   for (int i=1; i<argc; i++) {
      ...
      i++;
      if (i >= argc) return -1;
      printf("WITH_WORKERS: Spawning %dx%s...\n",n,argv[i]);
   ...
   printf("WITH_WORKERS: Identify...\n");
   MPMD.Identify();
   printf("WITH_WORKERS: Finalize MPI...\n");
   MPI_Finalize();
   return 0;
}
